...
class Symfony extends Framework implements DoctrineProvider, PartedModule
{
     ...
     *
     * @phpstan-return ($collector is 'events' ? EventDataCollector :
     ...
     *     ($collector is 'twig' ? TwigDataCollector : DataCollectorInterface
     * )))))))
     */
    ...
    protected function debugResponse($url): void
    {
        ...
        foreach ($collectors as $collector => $method) {
            if ($profile->hasCollector($collector)) {
                $collectorInstance = $profile->getCollector($collector);
                /** @phpstan-ignore-next-line */
                $this->$method($collectorInstance);
            }
        }
    }
